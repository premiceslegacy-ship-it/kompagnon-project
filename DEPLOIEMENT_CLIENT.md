# SOP Déploiement — Nouveau client Kompagnon
...
Client BTP
  ...
  └── OpenRouter + Mistral (IA)                        → clés Kompagnon partagées (voir §IA)
...
GitHub (1 seul repo)
    ...
    │
    ├── Kompagnon-Weber    → compile le code + injecte vars Weber    → app Weber
    ├── Kompagnon-Dupont   → compile le code + injecte vars Dupont   → app Dupont
    └── Kompagnon-Demo     → compile le code + injecte vars Demo     → app démo
```
...
```
Cloudflare Pages — Kompagnon-Weber
...
Cloudflare Pages — Kompagnon-Dupont
...
| C2 | Créer les 3 buckets Storage + RLS (`logos`, `chantier-photos`, `attachments`) | Supabase MCP | MCP connecté ✅ |
...
Pour la release actuelle, la migration supplémentaire à appliquer chez les clients existants est :
`047_catalog_dimension_pricing.sql`

Effets de cette migration :
- distinction catalogue entre `article` et `service`
- tarification dimensionnelle facultative sur les éléments de catalogue
- profil de prestation type (`article`, `service`, `mixed`)

Impact déploiement :
- obligatoire avant d'utiliser les nouveaux champs du catalogue
- pas de changement manuel Cloudflare requis pour cette feature
- pas de secret supplémentaire à injecter
...
| `attachments` | ❌ | PJ devis/factures | Auth + org uniquement |
...
OPENROUTER_API_KEY=sk-or-...          ← clé Kompagnon partagée
MISTRAL_API_KEY=...                    ← clé Kompagnon partagée
...
> **Prérequis critique :** `OPENROUTER_API_KEY` doit être injectée dans les variables Cloudflare Pages (voir §3). Le Worker appelle `/api/cron/auto-reminders` sur l'app, qui génère chaque email via l'IA. Si cette clé est absente ou invalide, **aucune relance ne part** (échec silencieux côté cron). C'est la clé Kompagnon partagée — elle est déjà dans ton `.env.local`.
...
- [ ] `SELECT count(*) FROM storage.buckets` → 3 buckets
...
**Stratégie Kompagnon :** SC connectée à B2Brouter (PA agréée). 1 clé API par client.
...
**Court terme (< 10 clients) → clé Kompagnon partagée + logging en DB**
...
**Aujourd'hui :** clé Kompagnon partagée. Variable `OPENROUTER_API_KEY` marquée "Oui (partagée)" dans le tableau ci-dessous.
...
Kompagnon préfinance, refacture avec ~20% de marge :
...
| `OPENROUTER_API_KEY` | openrouter.ai/keys | **Oui** (clé Kompagnon) |
| `MISTRAL_API_KEY` | console.mistral.ai | **Oui** (clé Kompagnon) |
...
|--------|-------------|---------|------------|------------|---------|------------|
| Weber Tôlerie (dev) | `pyxnmohknxmbpbcuvudg` | localhost | 2024 | 001→039 | ❌ | ❌ |
