...
GitHub (1 seul repo)
    │
    ├── Déploiement manuel/scripté → OpenNext compile puis Wrangler déploie
    │
...
- Un bugfix n'arrive pas tout seul chez tous les clients : il faut lancer `./scripts/deploy-all-clients.sh` après le push ou depuis ta machine.
- Une nouvelle feature avec migration nécessite d'abord `supabase db push` sur chaque Supabase client, puis le redéploiement app.
- `deploy-all-clients.sh` reconstruit et déploie les apps listées dans `scripts/clients.txt`. Le cockpit, les Edge Functions et les Workers cron restent séparés.
...
- `105` :
  - `organizations.setup_checklist_dismissed BOOLEAN DEFAULT false` — permet à l'owner de masquer la checklist de lancement du dashboard
- `106` :
  - nouvelle table `chantier_task_assignments` — assignation multiple des tâches chantier à une équipe ou à un membre terrain
  - obligatoire avant d'utiliser les assignations de tâches dans l'espace membre et les vues chantier enrichies
- `107` :
  - nouvelles tables `org_monthly_objectives` et `org_monthly_objective_customs`
...
`POST /api/cron/embeddings` est un endpoint Next.js. Il vectorise les lignes `company_memory` dont `embedding IS NULL`.

Deux options existent :
- `workers/embeddings` est un Cloudflare Worker dédié avec cron horaire (`0 * * * *`).
- cron-job.org peut aussi appeler directement l'endpoint si tu ne veux pas déployer ce Worker par client.

Déploiement Worker dédié :
```bash
...
wrangler deploy --name embeddings-<nomclient>
```

Planification externe alternative — cron-job.org :
