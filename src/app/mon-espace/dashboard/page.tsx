...
export default async function MonEspaceDashboardPage() {
  ...
  const [pointages, plannings, tasks, orgRow, icalToken, memberGoals, interventionsRows, chantiers, absencesRows] = await Promise.all([
  ...
  const upcomingAbsences = (absencesRows.data ?? []).map(a => ({
    id: a.id as string,
