    ...
    <main className="min-h-screen bg-[#070707] text-white">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
        ...
        <section className="mx-auto max-w-3xl py-14 text-center sm:py-20">
          ...
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.25em] text-accent">Votre atelier est prêt</p>
          <h1 className="font-display text-4xl font-extrabold tracking-tight sm:text-6xl">
            {companyName}, vos soirées peuvent redevenir les vôtres.
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-white/55 sm:text-lg">
            Découvrez ce qu’Atelier va prendre en charge. Activez votre accès pour transformer ces aperçus en vrais gains de temps.
          </p>
          ...
          {searchParams?.export && <p className="mx-auto mt-6 max-w-xl rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white/65">{searchParams.export === 'failed' ? 'L’export n’a pas pu être lancé.' : 'Votre export est en préparation. Vous recevrez le lien sécurisé par email.'}</p>}
          ...
          {canStartTrial && (
            <form action={startTrialAction} className="mt-8">
              <button className="inline-flex items-center gap-2 rounded-xl bg-accent px-6 py-4 font-bold text-black shadow-[0_0_35px_rgba(255,159,28,.18)] transition hover:brightness-110">
                Essayer Expert gratuitement pendant 14 jours <ArrowRight className="h-5 w-5" />
              </button>
              <p className="mt-3 text-xs text-white/35">Sans carte bancaire. Aucun prélèvement à la fin de l’essai.</p>
            </form>
          )}
          {membership.roleSlug !== 'owner' && <p className="mt-7 text-sm text-white/50">Le dirigeant du compte peut activer l’accès pour toute l’équipe.</p>}
        </section>

        <section className="grid gap-4 lg:grid-cols-3">
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="flex items-center gap-3"><MessageCircle className="h-5 w-5 text-accent" /><h2 className="font-display font-bold">Sarah suit vos relances</h2></div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="flex items-center gap-3"><FileText className="h-5 w-5 text-accent" /><h2 className="font-display font-bold">Un devis prêt plus vite</h2></div>
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
            <div className="flex items-center gap-3"><TrendingUp className="h-5 w-5 text-accent" /><h2 className="font-display font-bold">Votre marge, enfin visible</h2></div>
        ...
        {membership.roleSlug === 'owner' && (
          <section className="mx-auto mt-20 max-w-4xl">
            <div className="mb-8 text-center"><p className="text-xs font-bold uppercase tracking-[0.22em] text-accent">Accès immédiat pour toute l’équipe</p><h2 className="mt-3 font-display text-3xl font-extrabold">Choisissez votre rythme</h2><p className="mt-3 text-sm text-white/45">Sans frais de départ. Prix par entreprise, pas par utilisateur.</p></div>
            <div className="grid gap-4 md:grid-cols-2"><PlanCard tier="pro" price={69} /><PlanCard tier="expert" price={169} featured /></div>
          </section>
        )}
