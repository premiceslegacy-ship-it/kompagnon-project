  ...
  return (
    ...
    >
      {pending ? (
        ...
        <>Confirmer mon compte<ArrowRight className="w-4 h-4" /></>
      )}
    </button>
  )
}

export default function VerifyOtpForm({ email, mode }: { email: string; mode: 'signup' | 'recovery' }) {
  const action = mode === 'signup' ? verifyEmailOtp : verifyEmailOtp
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white/[0.04] backdrop-blur-[40px] border border-white/[0.08] rounded-3xl p-8 lg:p-10">
          ...
          <div className="mb-7 text-center">
            <h2 className="text-2xl font-bold font-display">Vérification email</h2>
